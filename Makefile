...
az-build: Dockerfile conda-env*.yml pip.conf fix-permissions docker-entrypoint
	@az acr build \
...
docker-build: Dockerfile conda-env*.yml fix-permissions docker-entrypoint
	@docker build \
	--build-arg CONDA_ENV_FILE=$(CONDA_ENV_FILE) \
	--build-arg VCS_URL=$(VCS_URL) \
	--build-arg VCS_REF=$(VCS_REF) \
